...
${NUMBER_ONE}=                   12
@{LIST_FRUITS}=                  mango    apple    grapes
@{LIST}=                    one    two    three
&{DICT_STUDENTS}=                key1=${STRING_HELLO}    key2=${NUMBER_ONE}    key3=@{LIST_FRUITS}
...
Call a keyword that returns a value
    ...
    Log to console    ${value}    # Return value

Do conditional IF - ELSE IF - ELSE execution
    IF    ${NUMBER} > 1
        Log to console    Greater than one.
    ELSE IF    "${STRING}" == "dog"
        Log to console    It's a dog!
    ELSE
        Log to console    Probably a cat.
    END

Loop a list
    Log to console    ${LIST}    # ['one', 'two', 'three']
    FOR    ${item}    IN    @{LIST}
        Log to console    ${item}    # one, two, three
    END
...
Loop a dictionary
    ...
    FOR    ${key_value_tuple}    IN    &{DICTIONARY}
        Log to console    ${key_value_tuple}
        # ('string', 'cat'), ('number', 1), ('list', ['one', 'two', 'three'])
    END
...
Loop a dictionary 2
    Log To Console    ${DICT_STUDENTS}
    FOR    ${item}    IN    &{DICT_STUDENTS}
        Log To Console    ${item}
        Log To Console    ${item}[0] ${item}[1]    #Using Index to read each value from the list
        # (('string', 'cat'), ('number', 1), ('list', ['one', 'two', 'three']))
    END
    # FOR    ${key}    IN    @{DICT_STUDENTS}
    #     Log To Console    ${key}=${DICT_STUDENTS}[${key}]
    #     # string=cat, number=1, list=['one', 'two', 'three']
    # END
    Log To Console    ==========================================
    FOR    ${value}    IN    @{DICT_STUDENTS}
        Log To Console    ${value}: ${DICT_STUDENTS}[${value}]
        # string=cat, number=1, list=['one', 'two', 'three']
    END
